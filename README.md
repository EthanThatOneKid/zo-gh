...
The webhook route can deploy `.zopack.md` files to a live Zo Space on push. Two triggers are wired in by default:

| Trigger                        | What it does                                                                                                                                  |
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `push` to `main`               | When any commit modifies a `*.zopack.md` file, the agent pulls the repo, extracts the route code, resolves `{{HANDLE}}`, and writes it to the space. |
| `workflow_dispatch`            | When a user clicks "Run workflow" in the GitHub Actions UI, an explicit sync is dispatched (no diff check — always syncs).                   |

The sync agent prompt is generated by `buildSyncPrompt()` in `webhook-agent/api-github-webhook.ts`. It expects the pack repo to live at `/home/workspace/code/github.com/<owner>/<repo>` and the destination handle to be `etok` (override the `HANDLE` constant if you fork for a different Zo account).

Repos that want auto-sync must:

1. Be a `git clone` on the Zo host at the expected path
2. Have a `*.zopack.md` file in the pack format (see [`zopack-cli`](https://github.com/EthanThatOneKid/zopack-cli))
3. Have the webhook registered via `scripts/register-webhook.sh` (so push events fire)
4. Have `ZO_API_KEY` set as a Zo secret

> **Tip:** If the repo has a `scripts/sync-space.sh`, the agent prefers running it over hand-extracting the pack. See [`zo-future-of-collaboration/scripts/sync-space.sh`](https://github.com/EthanThatOneKid/zo-future-of-collaboration) for an example.
